...
def get_cities(wikiurl=True,population=True,coordinates=True,alias=True,start=0,end=100000):
...
def wiki_api(title,props,lang="de"):
    ...
    url = "https://"+lang+".wikipedia.org/w/api.php?action=query&format=json&prop="+propstring+"&titles="+title
    props = [api_prop_answer.get(prop,prop) for prop in props]
    
    data_items = [(key,val) for key,val in json.loads(requests.get(url).text)["query"]["pages"].items()]
